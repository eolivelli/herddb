...
| Vector streaming compaction | `vector.index.compaction.streaming.enabled` (system property `herddb.vectorindex.streamingCompactionEnabled`) | `true` | Use jvector's `OnDiskGraphIndexCompactor` (issue #485) for IS-local compaction instead of the in-memory `GraphIndexBuilder` rebuild. The streaming engine bounds memory by `taskWindowSize × maxDegree` instead of `numTotalNodes × dimension`, so the historical 1 GB cap on `vector.index.compaction.maxBytes` is no longer dictated by heap pressure. Setting to `false` falls back to the legacy in-memory rebuild path (kept as an operator escape hatch). |
