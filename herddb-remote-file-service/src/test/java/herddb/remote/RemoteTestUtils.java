...
/**
 * Test utility methods for remote storage tests.
 */
class RemoteTestUtils {

    static DMLStatementExecutionResult executeUpdate(DBManager manager, String query,
        ...
        return (DMLStatementExecutionResult) manager.executePlan(
                translated.plan, translated.context, TransactionContext.NO_TRANSACTION);
    }

    static DataScanner scan(DBManager manager, String query,
            List<Object> parameters) throws StatementExecutionException {
        TranslatedQuery translated = manager.getPlanner().translate(
                TableSpace.DEFAULT, query, parameters, true, true, false, -1);
