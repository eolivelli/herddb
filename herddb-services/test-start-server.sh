...
export JAVA_OPTS="-XX:+UseG1GC -Duser.language=en -Xmx4g -Xms4g -Dio.netty.maxDirectMemory=0 -Djava.net.preferIPv4Stack=true -XX:MaxDirectMemorySize=1g -XX:+DisableExplicitGC -Djava.awt.headless=true -Djava.util.logging.config.file=conf/logging.properties --add-modules jdk.incubator.vector -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=$SERVER1DIR/server-heapdump.hprof -Djava.io.tmpdir=$(pwd)/tmp"
...
export JAVA_OPTS="-XX:+UseG1GC  -Duser.language=en -Dherddb.vectorindex.rebuild.threads=8  -Xmx40g -Xms40g -Djava.net.preferIPv4Stack=true -Djava.awt.headless=true -Djava.util.logging.config.file=conf/logging.properties --add-modules jdk.incubator.vector -Djava.io.tmpdir=$(pwd)/tmp"
