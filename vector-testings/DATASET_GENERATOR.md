...
## Standard Datasets on GCS (BIGANN, GIST1M)

The `StandardDatasetPublisher` tool stages a standard benchmark dataset (BIGANN
or GIST1M) in the flat layout that "custom" mode expects, generates the
descriptor JSON, and optionally uploads everything to Google Cloud Storage. The
purpose is to serve the datasets from the project-owned `gs://herddb-datasets`
bucket so that cloud runs do not depend on the TEXMEX FTP mirror.

### Descriptor `format` field

The descriptor JSON supports a `format` field:

- `"format": "fvecs"` — 4-byte float per dimension (GIST1M, SIFT1M, …).
  **Default** when the field is omitted, preserving the shape of descriptors
  generated by `DatasetGenerator`.
- `"format": "bvecs"` — 1 unsigned byte per dimension (BIGANN). Required to
  consume BIGANN under `--dataset custom`.

`totalVectors` and `numQueries` are parsed as `long`, so datasets beyond
`INT_MAX` vectors are supported.

### Publishing

```bash
# Stage GIST1M locally (downloads from FTP on first run).
./run_publish_standard_datasets.sh --dataset gist1m

# Stage BIGANN and push to gs://herddb-datasets/bigann/ in one go.
./run_publish_standard_datasets.sh --dataset bigann \
    --gs-path gs://herddb-datasets/bigann/
```

Flags:
- `--dataset bigann|gist1m` (required)
- `--dataset-dir <dir>` — where the native files live or will be downloaded
  (default: `./datasets`).
- `--output-dir <dir>` — staging directory (default:
  `./datasets/published/<dataset>`).
- `--gs-path <gs://…>` — when set, invokes `push_dataset_gcs.sh` after staging.

For BIGANN, the tool flattens the native `bigann_gnd/idx_1000000000.ivecs` to
`bigann_groundtruth.ivecs` so it sits alongside the descriptor.

### Consuming a published dataset

```bash
./run.sh --dataset custom \
    --dataset-url gs://herddb-datasets/gist1m/gist1m_descriptor.json \
    -n 1000000 --checkpoint
```
